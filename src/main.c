...
int main(int argn,char **arg)
{
	PGconn * hdlDataBase;
	ConnStatusType Status;
	PGTransactionStatusType stTransaccion;
	PGresult *queryDB;
	...
	/*-- consultamos el estado de las transacciones sobre la base de datos qTotem*/
	stTransaccion = PQtransactionStatus(hdlDataBase);
	switch(stTransaccion)
	{
		case PQTRANS_IDLE:
			printf("Actualmente en estado IDLE las trasnsacciones sobre la base de datos : \"%s\"\n"\
					,PQdb(hdlDataBase));
			break;
		case PQTRANS_ACTIVE:
			printf("Actualmente se esta ejecutando una transaccion sobre la base de datos : \"%s\"\n"\
					,PQdb(hdlDataBase));
			break;
		case PQTRANS_INTRANS:
			printf("Actualmente se estado IDLE en media de una transaccion sobre la base de datos : \"%s\"\n"\
					,PQdb(hdlDataBase));
			break;
		case PQTRANS_INERROR:
			printf("Actualmente se estado IDLE luego de una transaccion fallida  sobre la base de datos : \"%s\"\n"\
					,PQdb(hdlDataBase));
			break;
		case PQTRANS_UNKNOWN:
			printf("Fallo la coneccion con el servicio, al intentar realizar una transaccion sobre la base de datos : \"%s\"\n"\
					,PQdb(hdlDataBase));
			break;
	}
#endif
	...
	queryDB = PQexec(hdlDataBase, "BEGIN");
	if (PQresultStatus(queryDB) != PGRES_COMMAND_OK)
	{
		printf("BEGIN command failed: %s", PQerrorMessage(hdlDataBase));
		PQclear(queryDB);
		//printf("No se puede establecer coneccion con la base de datos: \"%s\"\n",PQdb(hdlDataBase));
		/*-- cerramos la coneccion y libreamos espacio, aunque la coneccion no sea sastifactoria.
		 *  Ya que se reserva memroria para el objeto "hdlDataBase" */
